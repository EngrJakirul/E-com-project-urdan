                                    ...
                                    <table class="table table-striped table-bordered table-hover" id="example-table" cellspacing="0" width="100%">
                                        ...
                                        @foreach($subCategories as $subCategory)
                                            <tr>
                                                ...
                                                <td>
                                                    <img src="{{asset($subCategory->image)}}" alt="" height="50" width="50">
                                                </td>
                                                <td>{{$subCategory->status == 1 ? 'Published' : 'Unpublished'}}</td>
                                                <td>
                                                    <a href="{{route('subcategory.edit', ['id' => $subCategory->id])}}" class="btn btn-sm btn-warning" onclick="return confirm('Are you sure want to edit this.....?')"><i class="fa fa-edit">Edit</i></a>
                                                    <a href="{{route('subcategory.delete', ['id' => $subCategory->id])}}" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure want to delete this....?')"><i class="fa fa-trash">Delete</i></a>
                                                </td>
                                            </tr>
                                        @endforeach
                                        </tbody>
